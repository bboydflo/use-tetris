...
import { Piece, rotatePiece } from './tetromino';
    ...
    return p.shape.every((row, rowIndex) => {
      return row.every((value, colIndex) => {
        ...
        if (value > 0 && this.state.grid[y] && this.state.grid[y][x] > 0) {
          if (__DEV__) {
            console.warn(
              `point (${y}, ${x}) is already filed with value ${this.state.grid[y][x]}`
            );
          }
          return false;
        }
  ...
  setCurrentPiece() {
    this.state.piece!.shape.map((row, rowIndex) => {
      row.map((value, colIndex) => {
        if (value >= 0) {
          this.state.grid[this.state.piece!.y + rowIndex][
            this.state.piece!.x + colIndex
          ] = value;
        }
      });
    });
    ...
    this.state.grid.forEach((row, rowIndex) => {
      if (row.every(value => value > 0)) {
        ...
        this.state.grid.unshift(
          Array.from({ length: this.columns }).fill(0) as number[]
        );
      }
    });
